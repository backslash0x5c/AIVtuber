...
if [[ -n "$obs_cmd" ]]; then
    echo "  ${obs_cmd//--websocket_password */--websocket_password ***}"
    if [[ $EUID -eq 0 && "$obs_cmd" != *--no-sandbox* ]]; then
        echo "  ❌ root実行なのに --no-sandbox がありません"
        echo "     → ブラウザソース(CEF)の子プロセスが起動できずクラッシュします"
    elif [[ "$obs_cmd" == *--no-sandbox* ]]; then
        echo "  ✓ --no-sandbox あり"
    fi
else
    echo "  (OBSプロセスが見つかりません)"
fi
